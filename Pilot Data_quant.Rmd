...
```
```{r}
"C:/Users/lb37426/Downloads"
WC_excel <- "../../Downloads/WC_AccessibilityPilot_.xlsx"
data <- "Participant Data"
d <- read_excel(WC_excel, sheet = data)
#d<-read_csv("../../Downloads/WC_AccessibilityPilot__.csv")
...
x |>
  ...
  ggplot(aes(x = type, y= ave_score)) +
   ...
   geom_errorbar(aes(x=type, ymin=ave_score-std, ymax=ave_score+std), width=0.4, colour="lightblue", alpha=0.9, linewidth=1.3)+
...
```{r}
library(cowplot)
combined_plot<-plot_grid(spires_plot, PIO_plot, Belonging_plot, ncol=3, align='v', labels=c('Attitude', 'Identity', 'Belonging'), label_size = 20)
...
ggsave("./Figures/Scores_plot.png", final_plot, width = 4000, height = 3000, units = "px")
