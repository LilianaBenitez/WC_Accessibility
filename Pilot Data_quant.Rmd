---
title: "Participatory and accessible ecological fieldwork impacts science identity and belonging for People with Disabilities- Quantitative analysis"
author: "Lili Benitez"
date: "2025-03-04"
...
w_SPIRES<-wilcox.test(d$Mean_SPIRES_post, d$Mean_SPIRES_pre, paired=TRUE) #warnings about exact p value with ties and zeros
summary(w_SPIRES) #V=675.5, p=9.695 e-07
...
wilcox.test(d$SUM_Belonging_post,d$SUM_Belonging_pre, paired=TRUE) #warnings about ties and zeros. V = 653.5, p-value = 5.374e-06
...
wilcox.test(d$PIO_post,d$PIO_pre, paired = TRUE) #warnings about ties and zeros, V = 388, p-value = 2.199e-05
...
d2 <- select(d, c(ID, Session, Gender_man, Gender_woman, Gender_NB, Race_W, Race_B, Race_N, Race_A,
                  Race_H, LGBT, Mean_SPIRES_post,gender,
...
d6<-d6%>%mutate (Disability_type=if_else(combined_text=="D_phy", "D_phy_only", 
                                         if_else(combined_text=="D_Chron", "D_chron_only", 
                                                 if_else(combined_text=="D_Men", "D_Men_only", 
                                                         if_else(combined_text=="D_Learn", "D_Learn_only", 
                                                                 if_else(combined_text=="D_Sen", "D_Sen_only", "combined"))))))
...
m_spires<-lmer(SPIRES_delta~Disability_type+(1|Session), data=d6)
summary(m_spires)
emmeans_spires<-emmeans(m_spires,~Disability_type)
pairs(emmeans_spires)
anova(m_spires)

m_PIO<-lmer(PIO_delta~Disability_type+(1|Session), data=d6)
summary(m_PIO)
emmeans_PIO<-emmeans(m_PIO,~Disability_type)
pairs(emmeans_PIO)
anova(m_PIO)


m_BEL<-lmer(BEL_delta~Disability_type+(1|Session), data=d6)
summary(m_BEL)
emmeans_BEL<-emmeans(m_BEL,~Disability_type)
pairs(emmeans_BEL)
anova(m_BEL)
...
```{r}
dis_plot_spires<-ggplot(data=d6,aes( x=Disability_type, y=SPIRES_delta))+
  geom_boxplot()+
  theme_classic()


dis_plot_pio<-ggplot(data=d6,aes( x=Disability_type, y=PIO_delta))+
  geom_boxplot()+
  theme_classic()

dis_plot_Bel<-ggplot(data=d6,aes( x=Disability_type, y=BEL_delta))+
  geom_boxplot()+
  theme_classic()
...
x |>
  ...
  ggplot(aes(x = type, y= ave_score)) +
    geom_point(stat = "identity", size=2, show.legend = FALSE, fill="grey")+
   geom_errorbar(aes(x=type, ymin=ave_score-std, ymax=ave_score+std), width=0.5, colour="black", alpha=1, linewidth=0.5)+
